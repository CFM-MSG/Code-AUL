...
def get_dist_launch(args):  # some examples
    if args.dist == 'f4':
        ...
        return "CUDA_VISIBLE_DEVICES=1,2 WORLD_SIZE=2 python3 -m torch.distributed.launch --nproc_per_node=2 " \
               "--nnodes=1 --master_port={:}".format(MASTER_PORT)

    elif args.dist == 'f-0':
        return "CUDA_VISIBLE_DEVICES=2,4,5 WORLD_SIZE=3 python3 -m torch.distributed.launch --nproc_per_node=3 " \
               "--nnodes=1 "
