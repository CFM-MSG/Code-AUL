...
def create_scheduler(args, optimizer):
    ...
    if args.sched == 'linear':
        ...
        def lr_lambda(current_step: int):
            if current_step < (args.num_warmup_steps - args.num_warmup_steps/2):
                tt = 1
            elif current_step < (args.num_warmup_steps):
                tt = 0.75
            elif current_step < (args.num_warmup_steps + args.num_warmup_steps / 2):
                tt = 0.5
            elif current_step < (args.num_warmup_steps * 2+ args.num_warmup_steps / 2):
                tt = 0.3
